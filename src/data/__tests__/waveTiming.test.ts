...
describe("wave timing", () => {
  it("uses a 20-second regular wave interval", () => {
    expect(WAVE_INTERVAL_SEC).toBe(20);
  });

  it("keeps the player-defined 5-second instant-wave cooldown", () => {
    expect(INSTANT_WAVE_TOKEN_COOLDOWN_AFTER_WAVE_SEC).toBe(5);
  });
  ...
  it("preserves the old 22/90 no-spend tail for AI instant-wave tokens", () => {
    ...
    expect(AI_INSTANT_WAVE_MIN_REMAINING_SEC).toBeCloseTo(20 * (22 / 90), 8);
