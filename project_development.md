...
> **2026-08-12 갱신 — 대회 제출 이후 현재 국면.** 제출물 5종은 마무리됐고,
> 지금 작업 축은 **게임 자체의 품질(랙 / AI 거동)과 온라인 PvP 준비**로
> 옮겨졌다. 아래 5번 본문은 그 이전(7월 말) 시점의 스냅샷이므로, 현재
> 우선순위는 `docs/dev-wiki/backlog.md`의 Active Queue 상단 3개 항목을
> 먼저 볼 것.
>
> - 랙 원인 2건(보이지 않는 유닛 이름표의 매 프레임 재래스터화, 구조물
>   비주얼 매 프레임 갱신)은 계측으로 특정해 수정 완료.
>   유닛 108기 기준 `tickCombat` 54.41ms -> 16.86ms.
> - "적 앞에서 안 때리고 서 있는" 증상은 별개 버그가 아니라 같은 뿌리였다.
>   전진은 deltaSec로 정규화돼 있었는데 전투 슬롯으로 파고드는 `laneRow`
>   보간만 프레임 수에 비례해서, 프레임이 떨어지면 교전 진입만 느려졌다.
>   `src/systems/lane-combat/frameLerp.ts`로 수정 완료.
> - **다음 판단은 사용자 플레이 확인 대기**: 실기에서 두 증상이 실제로
>   사라졌는지 확인한 뒤에 O(n^2) 전투 탐색 최적화 착수 여부를 정한다.
> - 온라인 PvP 목표 형태는 **1v1 실시간(서버 권위형)** 으로 확정.
>   선행 과제는 네트워크 코드가 아니라 **시뮬/렌더 분리** — `LaneUnit`이
>   시뮬 상태와 Phaser 표시 객체 8개를 한 타입에 들고 있고 전투 로직이
>   `unit.sprite.x/y`를 45곳에서 읽고 있어, 결정론적 헤드리스 시뮬레이션을
>   먼저 뽑아내야 한다.
