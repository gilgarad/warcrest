...
> **통합 상태 (2026-07-27):** 이 문서는 `f57303b`에서 독립 프로토타입을
> 만든 시점의 역사적 설계·조사 기록이다. 이후 게임 씬이 모두 단일
> `AudioSystem`으로 이관되었고, 동적 BGM, 전투/UI SFX, 게임 내 설정,
> 포커스·재시작 처리가 실제 게임에 연결되었다. 현재 구현과 검증 결과는
> [오디오 통합 검증](audio-integration-validation.md)을 기준으로 한다.
> 아래의 “미연결”, “musicController를 직접 호출 중” 표현은 f57303b 당시
> 상태를 설명하는 것이며 현재 런타임 상태가 아니다.
