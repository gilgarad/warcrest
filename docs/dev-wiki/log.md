...
## 2026-08-19 — 아트 재개발 2: 바깥 들판과 화면 가라앉힘

전장 바깥은 `grass-base` **한 장을 7000단위에 걸쳐 반복**하는 tileSprite였다.
벽지처럼 보인다 — 1cm마다 같은 풀잎이다. 512px짜리 들판 텍스처를 변형 타일에서
합성하고 그 위에 얼룩·맨땅·자갈을 흩뿌렸다. **가장자리를 넘는 표시는 반대편으로
이어 그려** 이음매에서 잘리지 않게 했다.

카메라가 고정되면서 화면 가장자리에 있는 것은 대전 내내 거기 있게 됐다. 주변을
가라앉히는 비네트를 넣었는데, **세 번 틀렸고 세 번 다 인게임에서만 드러났다.**

1. **월드 크기로 그렸다** — 카메라가 7000x3900의 가운데만 비추므로 어두워지는
   구간이 통째로 화면 밖이었다. 아무 효과가 없었다.
2. **정사각 텍스처를 16:9로 늘렸다** — 방사형이 아니게 되면서 평평한 가운데와
   감쇠 구간의 경계가 **전장 위에 사각형으로** 드러났다. 비네트가 없는 편이
   나은 상태였다.
3. **`setScrollFactor(0)`이면 될 줄 알았다** — 스크롤은 고정되지만 **카메라 줌은
   그대로 적용된다.** 1600x900 그라디언트가 685x385로 축소돼 화면 한가운데
   사각형으로 깔렸다. HUD가 멀쩡한 이유가 별도 UI 카메라(줌 1)라는 것을 그제서야
   확인했다 — 화면 픽셀로 재는 것은 거기에 있어야 한다.

세 번 모두 "그렸다고 생각한 것"과 "그려진 것"이 달랐고, 스크린샷이 아니었으면
어느 것도 몰랐을 것이다.

## 2026-08-19 — 아트 재개발 3: 시각 언어를 바꾸다

사용자 지적: 지형·배경이 **기존 것을 재조립한 느낌**이고, 요청은 전면 재개발이었다.
맞는 지적이다. 앞선 작업은 재질 4종·타일 규격·기존 팔레트 샘플링을 그대로 두고
경계와 장식만 손봤다. 정리이지 새 방향이 아니다.

근본 원인은 생성기가 **그리는 방법을 하나만 알았다**는 것이다 — "연속 노이즈에
색조". 팔레트를 바꿔도 같은 것이 나온다. 방법을 바꿔야 했다.

### 세 언어

| | 규칙 |
|---|---|
| `pixel` | 타일의 1/4 해상도, 5단계 램프 + 4x4 정렬 디더, NEAREST 확대. 표면이 **덩어리와 모티프**(풀잎·자갈·파도 띠)로 구성 |
| `flat` | 단색 면 + 굵은 키라인, 내부 질감 최소 |
| `painted` | 3배 해상도 방향성 붓질, 한쪽에서 오는 빛 |

`painted`는 확대 시 **타일 격자가 보인다** — 타일마다 같은 방향 조명이 반복돼서다.
채택하려면 그것부터 고쳐야 한다.

### `pixel` 채택

컨셉(레트로 RTS)과 기존 유닛 스프라이트 톤에 맞고, 폰 크기에서 형태가 가장
또렷하다.

**필터링이 관건이었다.** 16픽셀 모티프를 4배로 늘린 것이라 성격이 전부 가장자리에
있는데, Phaser는 기본으로 텍스처를 부드럽게 만든다 — 그러면 애써 만든 잎과 자갈이
다시 뭉개진다. 게임 전역 `pixelArt` 플래그 대신 **지형 텍스처만 NEAREST**로 지정했다.
유닛 스프라이트는 고해상도 원본을 축소하는 것이라 보간이 필요하다.

### 2026-08-19 — 픽셀 지형의 경계를 디더로

구조물 작업에 들어가기 전 인게임을 확대해 보니 판단이 달라졌다.

**유닛과 구조물도 실제로는 픽셀아트다.** 원본이 512px 렌더링이라 일러스트로
보였지만, 게임 크기에서는 하드 엣지와 디더링이 그대로 읽힌다. 지형과 톤은
오히려 맞는다 — 스타일 충돌을 걱정했는데 사실이 아니었다.

진짜 문제는 둘이었다:

1. **표면이 색종이처럼** 보였다. 자갈·흙 모티프가 몇 픽셀마다 하나씩 있어
   재질이 아니라 정적 노이즈로 읽혔다. 밀도를 절반으로 줄였다.
2. **각진 패치워크.** 타일 세트는 재질을 45도로 자르는데, 이전 아트는 그 절단을
   부드러운 블렌드로 가리고 있었다. 하드 엣지 픽셀 타일은 블렌드가 안 되니
   삼각형 조각들이 그대로 드러났다.

두 번째는 **픽셀아트의 정석대로 디더 경계**로 풀었다. 절단선을 따라 두 재질이
고정 패턴으로 교차하는 띠를 두면, 눈은 그것을 이음매가 아니라 점진적 변화로
읽는다. 타일 자체의 테두리(이웃과 공유하는 변)는 디더가 먹지 않도록 막았다 —
거기가 뚫리면 이웃 사이에 구멍이 생긴다.

## 2026-08-19 — 아트 재개발 4: UI 크롬

HUD가 단색 사각형 + 1px 테두리였다. 아래 지형이 평평할 때는 괜찮았지만, 지형이
픽셀아트가 되고 나니 **화면에서 디버그 오버레이처럼 보이는 유일한 요소**였다.

지형과 같은 언어로 9-슬라이스 프레임 5종을 만들었다(패널, 버튼, 호버, 비활성,
위험). 하드 엣지, 좌상단에서 오는 빛, 놋쇠 테두리, 모서리 스터드.

**첫 시안은 채워넣기가 틀렸다.** 지형처럼 디더 패턴으로 채웠더니 9-슬라이스가
가운데를 늘리면서 **거대한 체크무늬**가 됐다. 임의 배율로 늘려도 견디는 것은
단색뿐이므로, 성격은 **늘어나지 않는 테두리**가 져야 한다.

### 프레임과 히트 영역을 분리하면서 생긴 연쇄

Phaser의 nine-slice는 입력 대상이 아니라, 기존 사각형을 **투명한 히트 영역**으로
남겼다. 그런데 코드 곳곳이 `rect.visible`을 "화면에 있는가"로 읽고 있었고, 히트
영역은 설계상 항상 보이므로 그 판단이 전부 뒤집혔다:

- **비용 숫자가 버튼 없이 떠다녔다** — 원 작성자가 주석으로 정확히 경고해둔 실패를
  내가 되살렸다.
- **숨긴 버튼이 클릭을 가로챘다** — `isPointerOverActionButton`이 보이지 않는
  버튼에 대해 참을 반환.
- **모바일 하네스가 실패했다** — 볼 수 없는 컨트롤을 측정 대상에 넣음.

표시 판단을 전부 `frame.visible`로 옮기고, 숨길 때 히트 영역도 함께 숨기도록 했다.

### 덤으로 잡은 것

거점 액션 버튼(타워 재건·병참 등)이 **터치 크기 적용을 못 받고 있었다.** 별도
경로로 만들어져 첫 모바일 작업에서 누락됐고, 폰에서 19 CSS px — 기준의 절반도
안 됐다. 하네스가 프레임 교체 덕에 이제야 그것을 볼 수 있게 되면서 드러났다.

## 2026-08-19 — 아이콘 한 줄, 그리고 아래 레인 내리기

사용자 요청 둘이 서로 맞물려 있었다. "아래 레인을 더 내려라"와 "버튼을 아이콘화
해서 아래 한 줄로". 계산해 보니 프레이밍이 **가로에 걸려 있어**(zoomX 0.428 vs
zoomY 0.54) 세로 여유가 70 월드단위뿐이었다 — 레인을 먼저 내리면 줌이 떨어져
유닛이 작아진다. **아이콘부터 해야 했다.**

### 아이콘 한 줄

전략 액션 4개가 라벨 버튼 2줄이었다. 폰의 HUD 예산이 3줄인데 그중 둘을 쓰고
있었다. 32px 픽셀 아이콘을 그려 정사각 버튼 한 줄로 바꿨고, 밴드가
**670 → 784**로 내려가 필드가 114 게임단위 넓어졌다.

아이콘 두 개는 다시 그렸다. 뿔나팔은 **경사면**으로, 시대 업의 산형은 **전나무
두 그루**로 읽혔다 — 32픽셀에서는 물체의 그림이 아니라 기호여야 한다. 빨리감기
겹화살표와 계단을 오르는 화살로 교체했다.

아이콘 버튼에서는 라벨과 비용 행을 접었다. 안 그러면 그림 위에 "일꾼 고용"이
찍히고, 비용 5칸(210단위)이 102단위 정사각형의 세 배로 삐져나온다. 잃는 정보는
없다 — 못 사는 액션은 위험 프레임을 입고, 즉시 웨이브 토큰 수는 이미 상태
패널에 있다.

### 아래 레인

확보한 공간으로 간격을 **740 → 1010**으로 늘렸다. 줌 0.428과 유닛 66.5 CSS px는
그대로다.

두 레인이 중앙선 기준으로 더 이상 대칭이 아니다. 의도한 것이고 이유는 맵이
아니라 HUD다 — 상단 밴드가 156, 하단이 약 115라 필드에 남는 공간이 화면 중앙에
있지 않다. 카메라가 월드 중앙이 아니라 **내용**을 잡으므로 결과는 여전히 가운데
있는 것처럼 보인다.

### 테스트도 따라 고쳤다

일꾼 접기 테스트가 칩을 **텍스트로** 찾고 있었는데 칩이 아이콘이 되면서 실패했다.
아이콘 텍스처 키로 찾도록 바꿨다.

## 2026-08-19 — 상단 HUD를 걷어내다

요청: 상단 자원 바도 새 프레임으로, **두꺼운 배경은 없애고** 더 컴팩트하게.

걷어낸 것이 많았다. 상단은 그림 띠 하나, 화면 전폭 148단위 어두운 사각형,
구분선 두 줄, 자원 뒤의 두 번째 패널이 겹겹이 쌓여 있었다 — **텍스트 네 줄과
숫자 다섯 개를 보여주려고 전장의 3분의 1을 가리고 있었다.**

남긴 것은 조각들뿐이다. 글자 크기에 맞춘 상태 패널 하나, 자원마다 작은 프레임
하나. 그 사이로 전장이 보인다. 자원 칸에서 **라벨을 뺐다** — 아이콘이 이미 무엇인지
말하고, 숫자 위의 단어가 옛 바를 그렇게 높게 만든 주범이었다.

상단 밴드가 **156 → 150**으로 내려갔다.

### 두 곳이 각자 계산해서 겹쳤다

`소리` 버튼과 자원 행이 서로의 위치를 모른 채 각자 오른쪽 정렬을 하다가
**금속·연구 칩이 버튼 밑에 깔렸다.** 버튼 상자를 `screenLayout`의 공유 함수로
빼고 양쪽이 그것을 묻게 했다. 이 세션에서 같은 종류의 실수를 여러 번 했다 —
두 곳이 같은 것을 따로 계산하면 반드시 어긋난다.

## 2026-08-19 — 전장 파트의 비율을 키우다

요청: 길 너비와 유닛 등 **실제 전장 부분**의 비율을 키울 것(바깥 초록은 말고).

지렛대가 셋이었고, 프레이밍 예산 안에서 한계까지 올렸다:

| | 이전 | 이후 |
|---|---|---|
| 지형 셀 높이 | 96 | **112** |
| 길 코어(`stoneHalfRows`) | 1~2행 | **2~3행** |
| 유닛 크기 | 100css | **122css** |

유닛은 122가 상한이다. 130이면 내용 박스가 세로 예산(1481)을 넘어 줌이 떨어지고,
그러면 **키운 만큼 도로 작아진다.** 결과: 줌 0.428 유지, 화면상 유닛
**66.5 → 80.2 CSS px**.

길 코어를 넓힌 것은 수치를 보고 알았다 — `stoneHalfRows: 1`은 포장 도로가 한 행,
즉 **그 위에 선 병사보다 좁았다.**

끝 구간(본진 근처)에서는 두 레인의 지형 밴드가 겹친다. 맞는 결과다 — 거기서
레인이 실제로 만난다. 중간에는 초록이 약 114단위 남는다.
