...
- Tech stack: **not yet decided**. Do not assume a framework until an active
  backlog entry records the decision. Once chosen, update
  `docs/rules/testing.md` and `docs/patterns/README.md` with real commands
  and patterns.
- Team: solo/individual entry unless stated otherwise (affects submission
  item 5, which is skippable for individual entries per the contest rules
  screenshot).
